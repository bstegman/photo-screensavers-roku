...
Sub init()
    ...
    items = [
		{id:"LinkedGooglePhotosViewScreen", title:Locale_String("Start")},
...
Sub onLinkedButtons(evt as Object)
    ...
    if item.id = "unlink"
        ...
        Utils_Spinner_Show()
        API_Integrations_GooglePhotos_Revoke(callbackIntegrationRemove)
    else
...
Sub onModalUnlinkConfirm(evt as Object)
    ...
    if evt.getData() = "ok"
        ...
        if TYPE_isValid(m.integration)
            ...
            API_Integrations_GooglePhotos_Revoke(callbackRevoke, m.integration.access_token)
        else
