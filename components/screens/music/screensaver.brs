...
Sub _isActive(refresh=false as Boolean)
    ...
    genre = CreateObject("roSGNode", "ContentNode")
    genre.SetFields({
        id:m.top.input.genre.key,
        title:m.top.input.genre.title
    })
    m.sessionManager.musicGenre = genre

    if TYPE_isValidPath("AppInit.posterToken", m.global)

        m.viewPhoto.posterToken = m.global.AppInit.posterToken

        API_Collections_GetS3PhotosForIds(callbackGetPhotos, m.top.input.genre.photoCollectionId.toStr())
    else

        API_Collections_GetPhotosForIds(callbackGetPhotos, m.top.input.genre.photoCollectionId.toStr())
    end if
End Sub
