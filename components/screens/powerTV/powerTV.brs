...
Sub _isActive(refresh=false as Boolean)
    ...
    if TYPE_isValid(m.userManager.purchase)
        ...
        m.title.text = Locale_String("SubscribedMessage")
    else

        m.title.text = Locale_String("SubscribeMessage")
    end if
...
Sub createMenu()
    ...
    if NOT TYPE_isValid(m.userManager.purchase)

        item = content.createChild("ContentNode")
        item.SetFields({
            id:"subscribe",
            HDGRIDPOSTERURL:"pkg:/assets/images/icons/menu/subscribe.jpg",
            SHORTDESCRIPTIONLINE1:Locale_String("SubscribeNow")
        })
    end if
